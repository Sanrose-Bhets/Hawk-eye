...
function DraggableSeat({ seat, index, onRemove, onRename, highlight, studentName, readonly }: DraggableSeatProps) {
...
export function SeatCanvas({ seats, onChange, assignments, highlightStudent, readonly, showSeatCount = true }: SeatCanvasProps) {
  ...
  const isHighlighted = (index: number) => {
    ...
    const a = assignments.find((a) => a.seatIndex === index)
    return a?.studentName.toLowerCase().includes(highlightStudent.toLowerCase())
  }
  ...
  return (
    <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
      ...
      <div
        ...
        style={{ width: "100%", maxWidth: 700, minHeight: 500, minWidth: 700, overflow: "visible" }}
      >
        {seats.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center text-gray-400">
            {readonly ? "No seats in this plan" : "Click 'Add Seat' to start building your floor plan"}
          </div>
        )}
