...
export default function Home() {
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden py-12 px-6">
      <img
        src="/common/background.jpg"
        alt="Background"
        ...
        <div className="grid w-full max-w-5xl grid-cols-1 gap-6 md:grid-cols-3">
          {roles.map((role) => (
            <Link
              key={role.path}
              to={role.path}
              className="group relative flex flex-col justify-between rounded-[8px] border border-gray-200/80 bg-white/95 backdrop-blur-xs p-7 shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:border-primary/50 hover:shadow-xl cursor-pointer"
            >
