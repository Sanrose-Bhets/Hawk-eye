...
export default function ClassDetailPage() {
  ...
  const handleAutoAssign = useCallback(async () => {
    ...
    try {
      ...
      const thisClassEmails = new Set(cls.assignments.map((a) => a.studentEmail))
      ...
      const newStudents = [...available].sort((a, b) => a.name.localeCompare(b.name)).slice(0, remaining)
  ...
  const handleExport = useCallback(() => {
    ...
    const contentH = Math.max(seatsH + padding * 2 + titleH + titleGap, canvasMinH)
    ...
    if (seats.length === 0) {
      ...
      ctx.fillText("No seats in this plan", borderX + borderW / 2, borderY + borderH / 2 - 7)
      ctx.textAlign = "start"
    }
    ...
    seats.forEach((seat, i) => {
      ...
      ctx.fillText(seat.label, x + seatW / 2, y + (assignment ? 30 : seatH / 2))
    ...
    <div className="max-w-4xl">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={handleAutoAssign} disabled={saving}>
      ...
      {cls.assignments.length > 0 && (
        <Card className="mb-6">
          <CardHeader>
            <h2 className="text-base font-semibold text-gray-900">Assigned Students</h2>
          </CardHeader>
          <CardContent>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-gray-900 truncate">{a.studentName}</div>
                        <div className="text-xs text-gray-500 truncate">{a.studentEmail}</div>
                      </div>
                    </div>
                  )
                })}
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-gray-500">
                {DUMMY_STUDENTS.length - globalTakenEmails.size} available / {DUMMY_STUDENTS.length} total
              </span>
            </div>
          </CardHeader>
          <CardContent>
            <div className="max-h-80 overflow-y-auto rounded-xl border border-gray-200 divide-y divide-gray-100">
              {DUMMY_STUDENTS.sort((a, b) => a.name.localeCompare(b.name)).map((student) => {
                const isInThisClass = cls.assignments.some((a) => a.studentEmail === student.email)
                const assignedClassName = globalTakenEmails.get(student.email)
