...
export default function CreateClassPage() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <Card className="mb-6">
        <CardHeader>
          <h2 className="text-base font-semibold text-gray-900">Class Details</h2>
        </CardHeader>
        <CardContent className="space-y-5">
          ...
          <div className="space-y-2">
            <Label htmlFor="floorPlan" required>Floor Plan</Label>
            {floorPlans.length === 0 ? (
            ...
            <div className="rounded-xl border border-primary/30 bg-primary-light p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-medium text-gray-900">
                  {autoAssigned.length} students will be assigned ({availableStudents.length} available)
                </span>
              </div>
              {autoAssigned.length > 0 ? (
                ...
                <p className="text-sm text-gray-500">No available students left to assign</p>
              )}
            </div>
          )}
        </CardContent>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-gray-500">
                {availableStudents.length} available / {DUMMY_STUDENTS.length} total
              </span>
            </div>
          </CardHeader>
          <CardContent>
            <div className="max-h-80 overflow-y-auto rounded-xl border border-gray-200 divide-y divide-gray-100">
              {DUMMY_STUDENTS.sort((a, b) => a.name.localeCompare(b.name)).map((student) => {
                ...
                const isAssignedToNew = autoAssigned.some((a) => a.id === student.id)
                return (
                  <div
                    key={student.id}
      ...
      <div className="flex gap-3 mt-6">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/rte/classes")}>
          Cancel
        </Button>
      </div>
    </div>
